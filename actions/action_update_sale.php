...
if (!TrackItem::valid_code($dbh, $confirmCode)) {
    $session->add_message("error", "Invalid confirmation code");
    die(header('Location: ' . $_SERVER['HTTP_REFERER']));
}

if (TrackItem::update_shipping($dbh, $_POST["purchase"])) {
    $session->add_message('success', 'Shipping process updated successfully.');
}
else {
    $session->add_message('error', 'Unable to update process.');
}
